 ...
 */
var OrbitMesh = function(gl, vertexData) {
 ...
 */
OrbitMesh.prototype.initBuffers = function(gl, vertexData) {
 ...
 */
OrbitMesh.prototype.deleteBuffers = function(gl) {
    // Delete all buffers
 ...
 */
OrbitMesh.prototype.render = function(gl, uni, mat) {
    // If the buffers haven't been initialized, do nothing
    if( ! this.vao ) return;

    mat.setUniforms(gl,uni);

    // Bind to the VAO and draw the triangles
    gl.bindVertexArray(this.vao);
    gl.drawElements( gl.LINE_STRIP, this.nVerts, gl.UNSIGNED_INT, 0);
